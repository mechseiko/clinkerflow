...
import { X, AlertTriangle, CheckCircle, XCircle, MinusCircle, ChevronRight } from 'lucide-react';
...
function FlowArrow({ from, to, index }: { from: PlantStage; to: PlantStage; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <ChevronRight className="w-4 h-4 rotate-90" style={{ color }} />
      </motion.div>
    </motion.div>
  );
}

function DetailPanel({ stage, onClose }: { stage: PlantStage; onClose: () => void }) {
  const color = statusColor(stage.status);
  const Icon = STATUS_ICON[stage.status];
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 top-0 bottom-0 w-80 bg-[#080B12] border-l border-[#1E2536] overflow-y-auto z-20"
    >
      <div className="p-5">
        {/* Header */}
        <div className="flex items-start justify-between mb-4">
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg bg-[#1A2035] flex items-center justify-center text-slate-400 hover:text-white transition-colors"
          >
...
export function PlantFlowVisualization() {
  ...
  return (
    <div className="relative h-full min-h-[600px] overflow-hidden">
      <div className={`transition-all duration-300 ${selectedStage ? 'mr-80' : ''}`}>
      ...
      <AnimatePresence>
        {selectedStage && (
          <DetailPanel stage={selectedStage} onClose={() => setSelectedStage(null)} />
        )}
      </AnimatePresence>
    </div>
  );
